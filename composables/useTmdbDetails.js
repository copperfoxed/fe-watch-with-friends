...
export function useTmdbDetails({ id }) {
  ...
  const details = computed(() => {
    ...
    return {
      ...
      trailerEmbedUrl:
        trailer ? `https://www.youtube.com/embed/${trailer}` : null,
      trailerWatchUrl:
        trailer ? `https://www.youtube.com/watch?v=${trailer}` : null,
    };
  });
  ...
  const platforms = computed(() => {
    ...
    return {
      streamLogo:
        stream ? `https://image.tmdb.org/t/p/w92${stream.logo_path}` : null,
      streamName: stream?.provider_name || null,
      rentLogo:
        rental ? `https://image.tmdb.org/t/p/w92${rental.logo_path}` : null,
      rentName: rental?.provider_name || null,
      buyLogo:
        purchase ? `https://image.tmdb.org/t/p/w92${purchase.logo_path}` : null,
